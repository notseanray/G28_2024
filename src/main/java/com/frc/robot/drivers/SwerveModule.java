...
public class SwerveModule {
    ...
    public SparkPIDController mDrivePID;
    public SparkPIDController mAnglePID;
    ...
    public SwerveModule(int moduleNumber, SwerveModuleConstants moduleConstants) {
        ...
        if (moduleConstants.cancoderID < 0) {
            ...
            System.out.println("angle encoder");
        } else {
            System.out.println("failed to grab ecnoder");
            // TODO handle this properly with interface to abstract over
            // angleEncoder = new CANCoder(moduleConstants.cancoderID);
        }
        mAnglePID.setFeedbackDevice(mAngleMotor.getEncoder());
        configAngleEncoder();
        ...
        mDrivePID.setFeedbackDevice(mDriveMotor.getEncoder());
        configDriveMotor();
    ...
    public void setDesiredState(SwerveModuleState desiredState, boolean isOpenLoop) {
        ...
        mAnglePID.setReference(Conversions.degreesToNeo(angle, Constants.SwerveConstants.angleGearRatio), ControlType.kPosition);
        lastAngle = angle;
    }

    // TODO
    public void resetToAbsolute() {
        double absolutePosition = Conversions.degreesToNeo(getCanCoder().getDegrees() - angleOffset, Constants.SwerveConstants.angleGearRatio);
    ...
    public void updateAnglePID(double kP, double kI, double kD) {
        if (anglekP != kP) {
            anglekP = kP;
            mAnglePID.setP(anglekP);
        }
        if (anglekI != kI) {
            anglekI = kI;
            mAnglePID.setI(anglekI);
        }
        if (anglekD != kP) {
            anglekD = kD;
            mAnglePID.setD(anglekD);
        }
    }
    ...
    public Rotation2d getCanCoder() {
        return Rotation2d.fromDegrees(angleEncoder.getPosition() * 360.0);
    }
    ...
    public SwerveModuleState getState() {
        ...
        double velocity = Conversions.neoToMPS(mDriveMotor.getEncoder().getVelocity(),
                Constants.SwerveConstants.wheelCircumference, Constants.SwerveConstants.driveGearRatio);
        Rotation2d angle = Rotation2d.fromDegrees(Conversions.neoToDegrees(mAngleMotor.getEncoder().getPosition(), Constants.SwerveConstants.angleGearRatio));
